...
console.log(
  `${"#".padEnd(4)} ${"Código".padEnd(6)} ${"Producto".padEnd(12)} Stock  Estado`,
);
...
console.log(
  `Préstamo: $${montoPrestamo} | Tasa: ${tasaMensual * 100}% mensual | Cuotas: ${numeroCuotas}`,
);
...
console.log(
  `En meta (≥$${META_INDIVIDUAL}): ${vendedoresEnMeta}/${reporteVentas.length} vendedores`,
);
// ✅ Carlos   [Norte]  $3,200
// ✅ María    [Sur  ]  $4,750
// ⚠️ Luis     [Norte]  $2,100
...
for (const clave in configuracionSistema) {
  ...
  const valorMostrado =
    typeof valor === "boolean" ? (valor ? "Activado" : "Desactivado") : valor;
...
for (const producto of productos) {
  if (producto.stock <= STOCK_CRITICO) {
    console.log(
      `⚠️ Primer producto crítico encontrado: ${producto.nombre} (stock: ${producto.stock})`,
    );
