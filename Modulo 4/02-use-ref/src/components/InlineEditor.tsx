...
export default function InlineEditor() {
  ...
  const inputNew = useRef<HTMLInputElement>(null)
  const [saved, setSaved] = useState('Escribe algo y guarda')
  const [savednew, setSavednew] = useState('Escribe algo y guarda')

  function handleSave() {
    // Se lee el valor directamente del DOM — sin useState intermedio
    const value = inputRef.current?.value ?? ''
    const newValue = inputNew.current?.value ?? ''
    setSaved(value.trim() === '' ? '(vacío)' : value)
    setSavednew(newValue.trim() === '' ? '(vacío)' : newValue)
  }
  ...
  return (
    <div style={{ maxWidth: 340, display: 'flex', flexDirection: 'column', gap: 10 }}>
      <p style={{ margin: 0, color: '#6b7280', fontSize: 13 }}>
        Guardado: <strong style={{ color: '#111827' }}>{saved} {savednew}</strong>
      </p>
